...
pathAudiosEnfermos = "./audios/audios-unlam/audios-editados-renombrados/enfermos/U/"
pathAudiosSanos = "./audios/audios-unlam/audios-editados-renombrados/sanos/U/"
...
def crearEspectrogramaConEjes(path, file, carpetaDestino, off=0.0, dur=None):
    try: 
        ...
        librosa.display.specshow(librosa.power_to_db(spectrogram, ref=np.max), x_axis='time', y_axis='mel', fmax=8000, cmap=C_MAP)
...
def crearEspectrogramaLogConEjes(path, file, carpetaDestino, off=0.0, dur=None):
    try:
        ...
        plt.figure(figsize=(6,4))
        librosa.display.specshow(spectrogram, y_axis='log', sr=sr, hop_length=hop_length, x_axis='time', cmap=C_MAP)
        plt.colorbar(format='%+2.0f dB')        
        guardarGrafico(carpetaDestino, file)
    except Exception as e:
        print(e)
