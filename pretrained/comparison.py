...
def getModel():
  ...
  base_model = tf.keras.applications.ResNet50V2(input_shape=IMG_SHAPE, weights='imagenet', include_top=False)
...
def generateLog(batch_size, base_learning_rate, initial_epochs, acc_per_fold, loss_per_fold, test_dir, test_number):
  ...
  if not os.path.exists(sessionLog_dir):
